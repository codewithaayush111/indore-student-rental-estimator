...
if submitted:
    if not name.strip() or not email.strip() or not message.strip():
        st.error("Please enter your name, email and message.")
    else:
        try:
            payload = {
                "name": name.strip(),
                "email": email.strip(),
                "phone": phone.strip(),
                "preferred_property": preferred,
                "message": message.strip(),
                "_subject": "RENTIQ Indore - New Student Rental Inquiry",
                "_replyto": email.strip(),
                "_template": "table",
                "_captcha": "false",
                "_honey": "",
                "_url": "https://indore-student-rental-estimator-9f3ttt2vwdizappegp2dknn.streamlit.app/",
            }

            app_url = "https://indore-student-rental-estimator-9f3ttt2vwdizappegp2dknn.streamlit.app/"
            response = requests.post(
                "https://formsubmit.co/ajax/aayushnagar1743@gmail.com",
                json=payload,
                headers={
            ...
            # NEVER show success unless FormSubmit explicitly confirms success.
            success_value = result.get("success")
            if response.ok and str(success_value).lower() == "true":
                st.success("✅ Inquiry sent successfully! Check aayushnagar1743@gmail.com.")
            else:
                api_message = result.get("message") or result.get("error") or response.text[:500] or "No response details received."
                st.error("❌ Email was NOT confirmed as sent.")
                with st.expander("Show email service response"):
                    st.write(f"HTTP status: {response.status_code}")
                    st.code(str(api_message))
                if response.status_code in (401, 403):
                    st.warning("⚠️ FormSubmit activation/authorization is pending. Check Inbox + Spam of aayushnagar1743@gmail.com for the FormSubmit activation email, then activate the form and submit again.")
        except requests.Timeout:
            st.error("❌ Email service timed out. Please try again.")
        except requests.RequestException as exc:
            st.error("❌ Email service connection failed.")
