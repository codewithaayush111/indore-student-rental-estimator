    ...
    if not name.strip() or not email.strip() or not message.strip():
        ...
        try:
            ...
            if response.ok and result.get("success") is True:
                st.success("✅ Inquiry sent successfully! We will contact you soon.")
            elif response.ok and result.get("success") != False:
                st.success("✅ Inquiry submitted successfully! Please check the destination inbox.")
            else:
                st.error("❌ Unable to send the inquiry right now. Please try again.")
        except requests.RequestException:
            st.error("❌ Email service is temporarily unavailable. Please try again.")
